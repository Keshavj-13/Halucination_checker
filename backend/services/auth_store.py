...
SCHEMA_PATH = Path(__file__).resolve().parent.parent / "sql" / "init_app_state.sql"
...
class AuthStore:
    ...
    def ensure_default_admin(self) -> AuthenticatedUser:
        user, _ = self.ensure_user(DEFAULT_ADMIN_USERNAME, DEFAULT_ADMIN_PASSWORD)
        return user
    ...
    def get_history_detail(self, user_id: int, history_id: int) -> Optional[HistoryDetail]:
        with self._connect() as connection:
            row = connection.execute(
                """
                SELECT id, title, preview, created_at, source_name, audit_json
                FROM audit_history
